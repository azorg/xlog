...
// за единицу времени, т.н. Rate Limiter.
//
// Для параметров MaxNum и IntervalMs при активации функции ограничителя
// по умолчанию используются значения 100 и 1000 соответственно, что
// соответствует ограничению однотипных сообщений на уровне 100 шт/сек.
//
// Однотипными сообщениями считаются сообщения с одинаковым уровнем
// логирования (level) и одинаковым сообщением (msg).
//
// При превышении заданного лимита однотипные сообщение "группируются"
// путем объединения всех заданных атрибутов общий key/value список
// (последние значения переписывают более ранние) и добавляется
// дополнительный атрибут `repeated` целого типа, который
// указывает сколько сообщений было объедено в одно.
// В качестве метки времени сообщения передается время последнего сообщения.
//
...
// логгера, если у сообщений будет меняться основная строка, то такие
// сообщения не будут считаться однотипными.
// В целом этот эффект может использоваться и тогда, когда требуется
// обойти работу ограничителя для особо важной последовательности событий
// (в этом случае в текст сообщения нужно добавить изменяющуюся уникальную
// часть).
