...
## Пути вывода журнала
Определо несколько вариатов вывода журнала.
1. В никуда (если в Pipe="null", File="")
2. Только в pipe (stdout/stderr)
3. В кастомный io.Writer
4. В кастомный io.Writer и дополнтельно в pipe
5. В заданный файл
...
Выбор пути определяется полями Pipe, File, Rotation.Enable в
структуре конфигурации.

Для определения катомного io.Writer'а используются
отдельные функции (типа NewCustom()).
